...
export async function listPosts({ authorId, feedUserId, search = "", sortBy = "createdAt", order = "desc", limit = 50 } = {}) {
  const allowedSorts = new Set(["createdAt", "id"]);
  const followingIds = feedUserId ? await getFollowingIds(feedUserId) : null;
  const visibleAuthorIds = followingIds ? [Number(feedUserId), ...followingIds] : null;
