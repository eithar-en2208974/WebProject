...
async function main() {
  ...
  const users = Object.fromEntries(createdUsers.map((user) => [user.username, user]));
  ...
  const postData = [
    ...
    ["Maha", "The news feed feels better when there are real classmates to follow."],
    ["Maha", "Added comments to make the conversation section easier to demonstrate."],
    ...
    ["Yousef", "Stats are more interesting now that the seed file has enough data."],
    ["Aisha", "Shared a study update and checked that it appears on my profile."],
    ...
    ["Khalid", "Recording a short video will make the live presentation smoother."],
  ...
  await prisma.comment.createMany({
    data: [
      { postId: posts[0].id, authorId: users.Maha.id, text: "Good luck with the demo!" },
      { postId: posts[0].id, authorId: users.Omar.id, text: "Nice update." },
      { postId: posts[1].id, authorId: users.Layla.id, text: "The profile page looks clear." },
      { postId: posts[3].id, authorId: users.Nada.id, text: "The feed is working well." },
      { postId: posts[4].id, authorId: users.Sara.id, text: "More accounts make it feel real." },
      { postId: posts[6].id, authorId: users.Khalid.id, text: "The query examples are helpful." },
      { postId: posts[8].id, authorId: users.Omar.id, text: "Follow feature is useful." },
      { postId: posts[10].id, authorId: users.Aisha.id, text: "This will help with the video." },
      { postId: posts[12].id, authorId: users.Yousef.id, text: "I can see this on your profile." },
      { postId: posts[15].id, authorId: users.Maha.id, text: "Seed data saves so much time." },
      { postId: posts[17].id, authorId: users.Nada.id, text: "Agreed. A short video will help." },
      { postId: posts[19].id, authorId: users.Layla.id, text: "Messages are ready too." }
    ]
  });
  ...
  await prisma.message.createMany({
    data: [
      { senderId: users.Nada.id, receiverId: users.Maha.id, body: "Can you test the login page?" },
      { senderId: users.Maha.id, receiverId: users.Nada.id, body: "Yes, I will test it today." },
      { senderId: users.Omar.id, receiverId: users.Nada.id, body: "The profile page needs counts." },
      { senderId: users.Layla.id, receiverId: users.Maha.id, body: "The database examples look good." },
      { senderId: users.Yousef.id, receiverId: users.Nada.id, body: "I wrote a quick demo checklist." },
      { senderId: users.Aisha.id, receiverId: users.Sara.id, body: "Can you like my latest post?" },
      { senderId: users.Sara.id, receiverId: users.Aisha.id, body: "Done. I also added a comment." },
      { senderId: users.Khalid.id, receiverId: users.Omar.id, body: "Follow me so my posts appear in your feed." },
      { senderId: users.Maha.id, receiverId: users.Layla.id, body: "Let's record the stats changing after a post." },
      { senderId: users.Nada.id, receiverId: users.Yousef.id, body: "The seed file now has enough sample data." },
      { senderId: users.Malak.id, receiverId: users.Nada.id, body: "My profile bio is ready to test." },
      { senderId: users.Omar.id, receiverId: users.Khalid.id, body: "The most-liked post stat is ready to show." },
      { senderId: users.Layla.id, receiverId: users.Sara.id, body: "I checked the profile visibility scenario." }
    ]
  });
  ...
  console.log("Demo logins use password 123456, for example nada@example.com, maha@example.com, and malak123@hotmail.com.");
}
